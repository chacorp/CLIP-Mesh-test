...
def loop(config):
    ...
    idx = 0
    init_mesh = obj.load_obj(config["meshes"][idx])
    ...
    # if "displacement" in config["train_mesh_idx"][idx]:
    #     train_params += [displacement_map]
    ...
    optimizer  = torch.optim.Adam(train_params, lr=config["displacement_lr"])
    ...
    for it in t_loop:
        ...
        # if subdiv != None:
        #     lapl_funcs.append(regularizer.laplace_regularizer_const(load_mesh))
        # else:
        #     lapl_funcs.append(None)
        lapl_funcs = [regularizer.laplace_regularizer_const(m)]
        ...
        #     if fn_l is not None:
        #         lapls.append(fn_l.eval(params_camera))
        # import pdb;pdb.set_trace()
        # lapls_lambda = lambda x, lapl_funcs: [fn_l.eval(x) for fn_l in lapl_funcs if fn_l is not None]
        # lapls = lapls_lambda(params, lapl_funcs) # returns list
        ...
        t_loop.set_description("CLIP Loss = {:0.6f}".format( clip_loss.item()))
